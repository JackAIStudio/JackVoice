...
- 识别服务采用 BYOK：用户自行填写火山引擎豆包语音 APP Key
...
- APP Key 存入操作系统凭据库，不写入 `settings.json`
...
2. 打开 JackVoice“设置 → 识别”，填写 APP Key。
...
开发期可用 16 kHz、单声道、16 bit PCM 联调。探针默认读取系统凭据库中的 APP Key；也可以通过 `JACKVOICE_VOLC_API_KEY` 临时提供，且不会读取设置文件中的明文 Key：
...
开机启动、引导完成状态和悬浮窗位置等系统相关状态按 Bundle ID 分别保存。两版不能同时运行，以避免同时写入历史、词库和设置。

首次启动新版时，程序会尝试把旧目录 `~/Library/Application Support/com.jackvoice.app` 迁移到共享目录。
