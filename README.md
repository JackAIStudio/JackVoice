...
- 本机词库可通过 MCP 提供给本地 Agent；只读热词和替换词，不暴露历史、录音或 API Key
- 本地历史支持复制、播放、定位文件和逐条删除
- 每次听写都会优先生成本地 WAV，网络或识别失败不影响录音保存
- 实时识别失败后，可对本地录音重新转写；网络恢复后会自动补转未完成的记录
...
本地 Agent 可以读取同一份热词和替换词来校正转写或专有名词，不必打开 JackVoice 窗口。构建 `jackvoice-mcp`、注册 MCP 和安装 Skill 的说明见 [agent/README.md](agent/README.md)。当前只读，不会改词库。
...
正式发布以开发机为准：`npm run build:desktop:release` 在 `bundle/dmg/delivery` 产出 DMG、`SHA256SUMS.txt` 和 `release-manifest.json`，再以这三项创建 GitHub Release，标签形如 `v8.17.15` 且与 `package.json` 版本一致。

GitHub Actions 的 `Release macOS` 工作流不再监听 tag，仅保留手动触发的备用入口，避免凭据未配置齐全时每次发版都留下一次失败运行。若要改用 CI 发布，需在受保护的 `release` Environment 配置变量 `JACKVOICE_APPLE_TEAM_ID`，以及 Secrets：`APPLE_CERTIFICATE`、`APPLE_CERTIFICATE_PASSWORD`、`APPLE_API_ISSUER`、`APPLE_API_KEY`、`APPLE_API_PRIVATE_KEY`，并限制可部署标签、启用人工批准。工作流在创建 Release 前会拒绝覆盖同名 Release，因此它与本机发布只能择一使用。正式发布凭据不得提供给 Pull Request 工作流。
