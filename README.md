...
- 首页可直接打开录音文件夹；录音不会自动到期或删除
- macOS 可选在听写录音期间临时静音系统输出，停止录音后自动恢复
- 识别服务采用 BYOK：用户自行填写豆包语音 App Key
...
- 正式版 App Key 存入操作系统凭据库；开发版存入权限受限的隔离凭据文件，二者均不写入共享的 `settings.json`
- 转写文本、词库和偏好设置保存在本机应用数据目录
- 本地录音永久保存在应用数据目录，JackVoice 不执行自动清理
- 历史中的单条删除与“清空全部”只清理应用内索引；录音文件由用户在录音文件夹中自行管理
- 旧版本若曾把 App Key 写入设置文件，新版首次启动会迁移到系统凭据库并重写设置文件
...
2. 在首次引导或 JackVoice“设置 → 识别”中填写 App Key，然后点击“保存并验证”。配置成功后，普通界面只显示连接状态；需要时可点击“更换 App Key”。
...
```bash
export JACKVOICE_VOLC_API_KEY="你的开发用 App Key"
npm run dev
```
...
两版会看到并修改同一份历史、录音、词库和非秘密识别设置。App Key 仍按正式版/开发版隔离：正式版使用系统凭据库，开发版使用私有本地文件；引导完成状态、开机启动和悬浮窗位置等客户端状态也分别保存。
...
# 保留 App Key、历史、录音、词库和识别设置
npm run qa:first-run:dev

# 模拟连 App Key 都不存在的新用户；原开发版凭据会先备份再移走
