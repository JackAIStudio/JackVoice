...
pub fn replacements_path(dir: &Path) -> PathBuf {
...
pub fn apply_replacements(text: &str, rules: &[(String, String)]) -> String {
    apply_replacements_detailed(text, rules).rewritten
}

pub fn apply_replacements_detailed(text: &str, rules: &[(String, String)]) -> ReplacementTrace {
    ...
    let patterns: Vec<(Vec<char>, &str, &str)> = rules
        ...
        .map(|(from, to)| {
            (
                from.to_lowercase().chars().collect::<Vec<_>>(),
                from.as_str(),
                to.as_str(),
            )
        })
        .filter(|(from, _, _)| !from.is_empty())
        .collect();
    ...
    let mut applied: Vec<ReplacementRule> = Vec::new();
    while i < chars.len() {
        let mut best_len = 0usize;
        let mut best: Option<(&str, &str)> = None;
        for (pattern, from, to) in &patterns {
...
mod tests {
    ...
    #[test]
    fn apply_replacements_detailed_reports_unique_hits() {
        let rules = vec![
            ("Broll".into(), "B roll".into()),
            ("绘画".into(), "会话".into()),
        ];
        let trace = apply_replacements_detailed("Broll 和绘画，还有 BROLL", &rules);
        assert_eq!(trace.rewritten, "B roll 和会话，还有 B roll");
        assert_eq!(
            trace.applied,
            vec![
                ReplacementRule {
